...
attrs={};var
node=JX.$N('textarea',attrs);node.value=spec.value||'';return{node:node,get:function(){return node.value;},set:function(value){node.value=value;}};},_boolValue:function(string_value){switch(string_value){case'true':return true;case'false':return false;}return null;},_newForm:function(spec){var
questions_parsed=spec.questions;var
initial=spec.initial;var
questions=[];var
question_list=[];for(const
question
in
questions_parsed){var
question_id='question-'+Math.floor(Math.random()*1000000);var
value=questions_parsed[question];var
value_type=typeof(value);var
question_obj;if(value_type=="string"){question_obj=JX.$N('input',{className:'aphront-form-control phuix-form-form-textarea',type:'textarea',value:value,id:question_id,question:question,});}else
if(value_type=="boolean"){if(initial){value=null;}question_obj=JX.Prefab.renderSelect({null:'Select One',true:'Yes',false:'No',},value,{className:'phuix-form-form-select',id:question_id,question:question,value:value,},[null,true,false]);}questions.push(question_obj);var
label=JX.$N('label',{className:'phuix-form-form-label',htmlFor:question_id,},JX.$H(question||''));var
display=JX.$N('div',{className:'phuix-form-form-row',},[label,question_obj]);question_list.push(display);}var
node=JX.$N('div',{className:'phui-form-view phui-form-form-table'},question_list);return{node:node,get:function(){var
map={};for(var
ii=0;ii<questions.length;ii++){var
question_obj=questions[ii];if(question_obj.type=="select-one"){var
bool_val=this._boolValue(question_obj.value);map[question_obj.question]=bool_val;}else{map[question_obj.question]=question_obj.value;}}return map;},set:function(_value){},};},_newOptgroups:function(spec){var
