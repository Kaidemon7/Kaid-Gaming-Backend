console.log("LaggedAPI Revenue Share: v1.8.3");
...
function onPaidEventHandler(ilar_message) {
    try{
        window.parent.onPaidEventHandler(ilar_message);
    }catch(e){
        console.log(e);
    }
}
...
LaggedAPI.GEvents={start:function(adResponse){
  ...
  adBreak({
    ...
    name: 'start-game',
    beforeAd: () => { disableButtons(true);adConvertsion();},
    afterAd: () => { enableButtons();if(parent_campaign_id<1&&!isDevMode){adTimer=-5}else{adTimer=0}},
    adBreakDone: () => {
      try{
        adResponse();
      }catch(e){
